...
function buildQrUrl(fileUrl: string): string {
  // If fileUrl is already absolute (http/https), use it directly
  if (fileUrl.startsWith("http://") || fileUrl.startsWith("https://")) {
    return `https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=${encodeURIComponent(fileUrl.trim())}`;
  }
  // For relative URLs, we need the base URL — get from window.location
...
export default function AdminClassRoutineQrPage() {
  ...
  // Auto-generate QR when fileUrl changes
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader
        ...
        subtitle="Upload the routine file — QR code is generated automatically. Students scan to view."
      />
          ...
          <div className="flex flex-wrap items-start gap-6">
            {data.qrCodeUrl ? (
              <div className="relative h-32 w-32 overflow-hidden rounded border bg-white p-1 shadow">
                <Image src={data.qrCodeUrl} alt="Class Routine QR" fill className="object-contain" unoptimized />
              </div>
            ) : (
            ...
            <div className="flex-1 space-y-1">
              ...
              {data.fileUrl && (
                <a href={data.fileUrl} target="_blank" rel="noreferrer" className="text-sm text-blue-600 underline">View Routine File ↗</a>
              )}
              <p className="text-sm text-gray-500">
                Status: {data.published ? <span className="text-green-600 font-medium">Published</span> : <span className="text-red-500">Draft</span>}
              </p>
            </div>
            <button onClick={() => { setEditing(true); setFormKey((k) => k + 1); }}
              className="rounded border border-blue-300 bg-white px-4 py-2 text-sm font-medium text-blue-700 hover:bg-blue-50">
              Edit
            </button>
          </div>
        </div>
        ...
        <AdminFormCard title={data.id ? "Update Class Routine" : "Setup Class Routine"}>
          <form key={formKey} onSubmit={handleSubmit} className="max-w-lg space-y-4">
            ...
            <div className="rounded-lg border-2 border-blue-200 bg-blue-50 p-4 space-y-3">
              <p className="text-sm font-semibold text-blue-800">Upload Routine File — QR generates automatically</p>
              <FileUpload
                name="fileUrl"
