...
export default function AdminBestProgrammerPage() {
  ...
  async function fetchItems() {
    try {
      const result = await fetchJson<{ success: boolean; data: Item[] }>("/api/best-programmer");
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await fetchJson<{ success: boolean; message?: string }>("/api/best-programmer", {
  ...
  async function handleDelete(id: string) {
    ...
    const result = await fetchJson<{ success: boolean; message?: string }>(`/api/best-programmer?id=${id}`, { method: "DELETE" });
  ...
  async function handleTogglePublish(id: string, currentPublished: boolean) {
    ...
    if (!item) return;
    const result = await fetchJson<{ success: boolean }>("/api/best-programmer", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader
        title="Manage Programming Stars"
        subtitle="Display board-এ Programming Stars section-এ দেখাবে।"
      />

      {message && <AdminFlashMessage type={message.type} text={message.text} />}

      <AdminFormCard title={editingItem ? "Edit Programming Star" : "Add New Programming Star"}>
        <form key={formKey} onSubmit={handleSubmit} className="space-y-3">
          ...
          <textarea
            name="description"
            placeholder="Description / Achievement (optional)"
            rows={3}
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              disabled={formLoading}
            />
            <label htmlFor={`pub-bp-${formKey}`} className="text-sm text-gray-700">
      ...
      <AdminListCard title={`All Programming Stars (${items.length})`}>
        {loading ? (
          <p className="text-gray-500">Loading...</p>
        ) : items.length === 0 ? (
          <p className="text-gray-500">No programming stars yet. Add one above.</p>
        ) : (
