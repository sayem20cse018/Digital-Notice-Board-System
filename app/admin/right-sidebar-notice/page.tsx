...
export default function AdminRightSidebarNoticePage() {
  ...
  async function fetchItems() {
    try {
      const result = await fetchJson<{ success: boolean; data: Item[] }>("/api/right-sidebar-notice");
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const data = {
      ...
      fileUrl: formData.get("fileUrl")?.toString().trim() || null,
  ...
  async function handleDelete(id: string) {
    ...
    const result = await fetchJson<{ success: boolean; message?: string }>(`/api/right-sidebar-notice?id=${id}`, { method: "DELETE" });
  ...
  async function handleTogglePublish(id: string, currentPublished: boolean) {
    ...
    if (!item) return;
    const result = await fetchJson<{ success: boolean }>("/api/right-sidebar-notice", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader
        title="Manage Update Notice"
        subtitle="Shown in the Update Notice section on the right panel of the display board. Add notices with images or files."
      />

      {message && <AdminFlashMessage type={message.type} text={message.text} />}

      <AdminFormCard title={editingItem ? "Edit Update Notice" : "Create New Update Notice"}>
        <form key={formKey} onSubmit={handleSubmit} className="space-y-3">
          ...
          <FileUpload name="fileUrl" label="Attachment File (PDF/Word/Excel — optional)" currentFile={editingItem?.fileUrl} />
          <div>
            <label className="mb-1 block text-xs text-gray-600">Display Order (lower number = shown first)</label>
            <input
              name="displayOrder"
              type="number"
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              disabled={formLoading}
            />
            <label htmlFor={`pub-sn-${formKey}`} className="text-sm text-gray-700">
      ...
      <AdminListCard title={`All Update Notices (${items.length})`}>
        {loading ? (
          <p className="text-gray-500">Loading...</p>
        ) : items.length === 0 ? (
          <p className="text-gray-500">No notices yet. Add one above.</p>
        ) : (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {item.fileUrl && (
                        <a href={item.fileUrl} target="_blank" rel="noreferrer" className="text-xs text-blue-600 underline">📎 Attachment</a>
                      )}
